...
def get_depth_info(cx, cy, point_cloud, tr_np):
    ...
    if not np.isnan(distance) and not np.isinf(distance):
        # code renders weird so i had to put spaces
        print("                                        Distance to gate {:1.3} m".format(distance), end="\r")
        # return [x, y, distance]
    else:
        ...
        if err == sl.ERROR_CODE.SUCCESS :
            ...
            for contour in contours:
                ...
                if area > 600:
                    ...
                    array_of_centers = draw_shapes_on_screen(image_ocv, cx, cy, depth_image_ocv, centers)
                    # print(len(centers))
                    
                    if len(array_of_centers) == 2:
                        # if there are two objects, it will get the center points of each object and plot them to create the distance line
                        dx = array_of_centers[0][0] - array_of_centers[1][0]
                        start_point = (array_of_centers[0][0], array_of_centers[0][1])
                        end_point = (array_of_centers[1][0], array_of_centers[1][1])
                        dy = array_of_centers[1][0] - array_of_centers[1][1]
                        #calculate lenght of line
                        center_line = np.sqrt(dx*dx+dy*dy)
                        #calculate center point of line
                        center_point_x = int((array_of_centers[0][0]+array_of_centers[1][0])/2)
                        center_point_y = int((array_of_centers[0][1]+array_of_centers[1][1])/2)
                        ...
                        #print(center_point)
                        cv2.circle(image_ocv, (center_point_x, center_point_y), 10, (255, 255, 255), -1)
                        print(f"Center of gate Coodinates: {center_point}")
                        cv2.line(image_ocv, start_point, end_point, (255,255,255), 2)
